...
along a single line of sight. Both share the same foam suppression form, with the temporal decorrelation
$C_\alpha(\tau;D)$ replaced by its spatial analog $C_\alpha(B;D)$:
\begin{equation}
...
$\lambda/B$. Lieu \&
Hillman~\cite{LieuHillman2003} implicitly assumed the large-baseline limit
$B\gg r_\perp$, where the suppression reaches its maximum $e^{-\sigma_\phi^2}$: their
...
extends their binary criterion to a continuous suppression curve
parameterized by $C_\alpha(B;D)$.
