...
temporal decorrelation measuring how the foam changes
between two observations separated by time $\tau$, maps to the spatial
decorrelation, measuring how
correlated the foam fluctuations are at two aperture points separated by
baseline $B$. In both cases the foam suppression has the same form: the spatial case
uses the spatial decorrelation function $C_\alpha(B;D)$ in place of the
temporal $C_\alpha(\tau;D)$:
\begin{equation}
  \mathcal{F}_{\rm foam}^{\rm spat}(B)
    \equiv e^{-\sigma_\phi^2[1-C_\alpha(B;D)]}.
  \label{eq:Ffoam_spat}
\end{equation}
This mirrors $\mathcal{F}_{\rm foam}(\tau)$ exactly: $C_\alpha(0;D)=1$ gives no
suppression at zero baseline, while $C_\alpha(B;D)\to 0$ for
...
\begin{equation}
  V(B) = V_{\rm src}(B)\cdot\mathcal{F}_{\rm foam}^{\rm spat}(B)
       = V_{\rm src}(B)\cdot e^{-\sigma_\phi^2[1-C_\alpha(B;D)]},
  \label{eq:visibility}
\end{equation}
...
shape encodes the spatial decorrelation $C_\alpha(B;D)$ --- in direct
analogy to how $g^{(1)}_{\rm obs}(\tau)$ encodes the temporal
decorrelation $C_\alpha(\tau;D)$.
...
distribution and $S=e^{-\sigma_\phi^2}$.
