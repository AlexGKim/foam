...
short-lag coherence.
Note that both closed forms in the weak-jigger limit share the general product structure
$g^{(1)}_{\rm obs}(\tau)=g^{(1)}_{\rm src}(\tau)\,F_{\rm foam}(\tau)$.

% [Author: I doubt Narrow-Band limit is needed in this article]
...
analogue of the Ng--Perlman Strehl-ratio bound~\cite{NgPerlman2022}:
...
the source also hosts a Compton-thick AGN resolved by strong Iron
...
optical line at the same distance, yielding the strongest available
...
i.e.\ $\sigma_\phi^2\lesssim 1$.  The cumulative phase variance follows
...
reproducing Ng \& Perlman~\cite{NgPerlman2022} Eq.~(4):
\begin{equation}
...
optical entry $\alpha>0.53$ of Ref.~\cite{NgPerlman2022} Table~1
(evaluated at $\lambda=5000$~\AA\ and $D=10$~Gpc).  The improvement
is not incidental: because $\sigma_\phi^2\propto\lambda_{\rm obs}^{-2}$,
...
$|\Delta\alpha|\lesssim 0.01$.  The result does not rule out the
...
3C\,273~\cite{GRAVITY2018}.  Methodologically the test is independent of imaging: it operates
...
and no constraint can be placed.  This is the same dynamical-versus-quasi-static
dichotomy that governs the Strehl test: an imaging Strehl measurement
on a single short exposure ($T_{\rm exp}\ll\tau_{\rm foam}$) sees a
frozen common-mode phase and recovers $\mathcal{S}\approx 1$, whereas
a long-exposure or ensemble-averaged Strehl ($T_{\rm exp}\gg\tau_{\rm foam}$)
recovers the full suppression $e^{-\sigma_\phi^2}$.  A spectroscopic
