...
aperture-plane coherence function $C_\alpha^{\mathrm{sp}}(B;D)$. We
emphasize that $C_\alpha^{\mathrm{sp}}(B;D)$ is distinct from the
source-plane transverse coherence $\Phi(b/r_\perp)$ of Section~\ref{sec:Calpha_origin},
which governs cross-ray correlations between rays from different
angular directions on the source; $C_\alpha^{\mathrm{sp}}$ instead
parameterizes how correlated the cumulative foam-induced phase is
between two points in the local wavefront at the observer.
...
in the resolved limit, differing only in whether the baseline is
realized internally (single aperture) or externally (two-element
interferometer). This unification places the published imaging
...
through X-ray and $\gamma$-ray observations, and mirrored here by the
spectroscopic bound derived in Section~\ref{sec:bzk4892}; (ii) baseline-resolved
...
3C\,273~\cite{GRAVITY2018}.  Methodologically the test is independent:
it operates entirely in the spectral domain and probes line-profile
contrast rather than image formation or fringe visibility, providing an
orthogonal cross-check on imaging-based constraints.
